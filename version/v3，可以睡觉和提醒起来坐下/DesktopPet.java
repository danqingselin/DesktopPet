...
public class DesktopPet extends JFrame {
    ...
    private static final int ROAM_COOLDOWN_MIN = 30;
    private static final int ROAM_COOLDOWN_MAX = 90;
    ...
    private static final int ROAM_CLIMB_MIN    = 120;
    ...
    // —— 新增：鼠标全局空闲阈值（1分钟） —— //
    private static final long MOUSE_IDLE_MS = 60_000L;
    ...
    private static final int REMIND_MINUTES = 30;           // 每30分钟一个提醒
    ...
    public DesktopPet() {
        ...
        bubbleWin.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                hideBubbleAndAdvance(); // 点掉气泡，进入下一阶段并重置计时
            }
        });
    ...
    private void onTick() {
        ...
        switch (state) {
            ...
            case IDLE: {
                Rectangle wa = getWorkArea();
                int floorY = wa.y + wa.height - SIZE;
                if (winY < floorY && surfaceLatchTicks <= 0) { state = State.FALL; yVel = 0; frameIndex = 0; break; }
                int left = wa.x, right = wa.x + wa.width - SIZE;
                winX = Math.max(left, Math.min(winX, right));
                winY = floorY;
                setLocation(winX, winY);
            ...
            case WALK_TO_RIGHT: {
                Rectangle wa = getWorkArea();
                int floorY = wa.y + wa.height - SIZE;
                if (winY < floorY - 1 && surfaceLatchTicks <= 0) { state = State.FALL; yVel = 0; frameIndex = 0; break; }
    ...
    public void startClimbLeft()  {
        ...
        if (state == State.CEILING) {
            ...
            if (winY < floorY && state != State.FALL) {
                state = State.FALL; yVel = 0; afterLand = AfterLand.WALK_TO_LEFT_CLIMB;
            } else {
                state = State.WALK_TO_LEFT; frameIndex = 0;
            }
        }
    ...
    public void startClimbRight() {
        ...
        if (state == State.CEILING) {
            ...
            int floorY = wa.y + wa.height - SIZE;
            if (winY < floorY && state != State.FALL) {
                state = State.FALL; yVel = 0; afterLand = AfterLand.WALK_TO_RIGHT_CLIMB;
            } else {
                state = State.WALK_TO_RIGHT; frameIndex = 0;
            }
        }
    ...
    // 让气泡跟随宠物，并保证在当前显示器工作区内；顶上放不下就放底部
    private void positionBubble() {
        ...
        int bx = getX() + BUBBLE_OFFSET_X;
        int byTop = getY() - (bh + BUBBLE_OFFSET_Y);             // 优先顶部
        int byBottom = getY() + getHeight() + BUBBLE_OFFSET_Y;   // 顶部放不下 → 底部
        int by = (byTop < work.y) ? byBottom : byTop;

        // 夹取到工作区
        if (bx < work.x) bx = work.x;
        if (bx + bw > work.x + work.width) bx = work.x + work.width - bw;
        if (by < work.y) by = work.y;
        if (by + bh > work.y + work.height) by = work.y + work.height - bh;
